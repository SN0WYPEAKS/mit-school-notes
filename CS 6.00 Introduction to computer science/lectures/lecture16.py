...
class Person(object):
    ...
    def __cmp__(self, other):
        return cmp((self.family_name, self.first_name), # type: ignore
                   (other.family_name, other.first_name)) # type: ignore
    def __str__(self):
...
class MITPerson(Person):
    ...
    def __cmp__(self, other):
        return cmp(self.idNum, other.idNum) # type: ignore
...
class Prof(MITPerson):
    ...
    def say(self, toWhom, something):
        if type(toWhom) == UG:
            ...
            return self.lecture(something) # type: ignore
...
class Faculty(object):
    ...
    def __iter__(self):
        ...
        return self
    def next(self):
        if self.place >= len(self.names): # type: ignore
            raise StopIteration
        self.place += 1 # type: ignore
